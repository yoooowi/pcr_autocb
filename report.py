...
from ..clanbattle_simple.dao import MemberDao, RecordDao
from ..clanbattle_simple.cbsimple import get_start_end_date
...
async def get_data_from_db(qqid):
    result = {
        'code': 1,
        'msg': '',
        'nickname': '',
    ...
    memberDB = MemberDao()
    recordDB = RecordDao()
    nickname = memberDB.get_name_from_qq(qqid)
    ...
    records = recordDB.get_member_monthly_record(nickname, start)
    if not records:
...
async def send_report(bot, event, background = 0):
    ...
    result = await get_data_from_db(uid)
